...
function toggleCart() {
    ...
    console.log('Current state - isOpening:', isOpening);
    ...
    setTimeout(() => {
        isToggling = false;
        console.log('Toggle completed - isOpen:', cartOverlay.classList.contains('active'));
    }, 300);
}
...
function closeCart() {
    ...
    if (cartOverlay && cartSidebar && cartOverlay.classList.contains('active')) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Cart elements:', { cartButton, cartIcon, cartOverlay, cartSidebar });
    
    // Function to handle cart toggle with debounce
    let isToggling = false;
    ...
    // Debug: Log all cart buttons
    const cartButtons = document.querySelectorAll('.cart-icon, .cart-button, [data-cart-toggle]');
    console.log('Found cart buttons:', cartButtons);
    
    // Add click event to all cart buttons
    cartButtons.forEach(button => {
        console.log('Adding click event to button:', button);
        button.removeEventListener('click', handleCartToggle); // Remove any existing listeners
    ...
    if (cartOverlay) {
        cartOverlay.addEventListener('click', function(e) {
            if (e.target === cartOverlay) {
                console.log('Closing cart via overlay');
                closeCart();
            }
        });
    }
    ...
    if (closeCartBtn) {
        closeCartBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('Closing cart via close button');
            closeCart();
        });
    }
