...
function initProductPage(familyOrProductId, productId = null) {
    ...
    console.log('initProductPage called with productId:', productId);
    ...
    if (item.products && typeof item.products === 'object') {
        // Ez egy termékcsalád (mint az irisz)
        console.log('This is a product family');
        ...
        console.log('Display data images:', displayData.images);
        console.log('Current product images:', currentProduct?.images);
        console.log('Family base images:', item.images);
    ...
    const productFamily = window.products?.irisz;  // Termékcsalád

    console.log('Product family:', productFamily);
    console.log('Display data:', displayData);
    // Eredeti adat ellenőrzése
    const originalData = window.products.irisz.products.irisz;
    console.log('EREDETI adat:', originalData);
    console.log('EREDETI video:', originalData.video);
    console.log('EREDETI keys:', Object.keys(originalData));
    
    // displayData honnan jön?
    console.log('=== DISPLAYDATA FORRÁS ===');
    // Itt keresd meg, honnan származik a displayData!
    
    // A jelenlegi kódodban valahol így kell lennie:
    // const displayData = valamilyen_forrás;
    
    // Helyette használd közvetlenül az eredeti adatot:
    const displayData = originalData;  // KÖZVETLEN HASZNÁLAT
    
    console.log('MÓDOSÍTOTT displayData:', displayData);
    console.log('MÓDOSÍTOTT video:', displayData.video);
    console.log('MÓDOSÍTOTT keys:', Object.keys(displayData));
    
    // Most már működnie kell:
    if (displayData.video) {
        console.log('✅ SIKERES VIDEÓ ELÉRÉS!');
        renderVideoElement(displayData);
    } else {
        console.log('❌ Még mindig nincs videó');
    }
    
    if (displayData.video) {
        console.log('📹 Videó renderelés indítása...');
        console.log('Video src:', displayData.video.src);
        console.log('Video poster:', displayData.video.poster);
    ...
    if (mainImage && displayData.images && displayData.images.length > 0) {
        // Set first image as main
        mainImage.src = displayData.images[0].full || '/energix/assets/placeholder.svg';
        mainImage.alt = displayData.name;
        mainImage.dataset.full = displayData.images[0].full; // Store full size URL
