...
function initProductPage(familyOrProductId, productId = null) {
    ...
    if (thumbnails && displayData.images && displayData.images.length > 0) {
        ...
        displayData.images.forEach((image, index) => {
            ...
            img.addEventListener('click', function() {
                ...
                if (!mainElement) {
                    console.error('❌ productImage elem nem található');
                    return;
                }
    
                // KRITIKUS: Prevent multiple rapid clicks
                if (mainElement.classList.contains('transitioning')) {
                    console.log('⚠️ Transition folyamatban, kattintás kihagyva');
                    return;
                }
                ...
                // SCOPE CHECK ITT
                console.log('handleVideoToImageTransition available:', typeof handleVideoToImageTransition);
                console.log('handleImageToImageTransition available:', typeof handleImageToImageTransition);
    
                // Transition flag beállítása
                ...
                if (mainElement.tagName === 'VIDEO') {
                    console.log('🎬 VIDEO elem észlelve');
                    if (typeof handleVideoToImageTransition === 'function') {
                        console.log('✅ Calling handleVideoToImageTransition');
                        handleVideoToImageTransition(mainElement, imageData);
                    } else {
                        console.error('❌ handleVideoToImageTransition nincs definiálva!');
                    }
                } else if (mainElement.tagName === 'IMG') {
                    console.log('🖼️ IMG elem észlelve');
                    if (typeof handleImageToImageTransition === 'function') {
                        console.log('✅ Calling handleImageToImageTransition');
                        handleImageToImageTransition(mainElement, imageData);
                    } else {
                        console.error('❌ handleImageToImageTransition nincs definiálva!');
                    }
                } else {
    ...
    initializeProductReviews(displayData);
}

// Videó → Kép átmenet optimalizált kezelése
function handleVideoToImageTransition(videoElement, imageData) {
    console.log('🔄 VIDEO → IMG váltás (optimalizált)');

    // Új kép elem előkészítése
    ...
    // KRITIKUS: Kép előtöltés és error handling
    newImg.onload = function() {
        console.log('✅ Új kép betöltődött, csere végrehajtása');
        ...
        setTimeout(() => {
            // KRITIKUS: NULL CHECK hozzáadása
            if (videoElement && videoElement.parentNode) {
                console.log('✅ Parent node elérhető, csere végrehajtása');
            
                // DOM csere
                videoElement.parentNode.replaceChild(newImg, videoElement);
            
                // Fade in új kép
                newImg.style.transition = 'opacity 0.2s ease';
                setTimeout(() => {
                    newImg.style.opacity = '1';
                
                    setTimeout(() => {
                        if (newImg) { // További NULL check
                ...
                console.log('✅ VIDEO → IMG váltás befejezve');
            
            } else {
                console.error('❌ VideoElement vagy parentNode nem elérhető');
            
                // Fallback: Kép hozzáadása a konténerhez
                const container = document.querySelector('.product-hero') || 
                                 document.querySelector('.product-images') ||
                                 document.body;
            
                if (container) {
                    // Régi elem eltávolítása ha létezik
                    ...
                    console.log('✅ Fallback: kép hozzáadva konténerhez');
                }
            }
    ...
    newImg.onerror = function() {
        ...
        // Fallback: visszaállítás eredeti állapotra
        videoElement.classList.remove('transitioning');
        videoElement.style.opacity = '1';

        // Placeholder kép használata
        newImg.src = '/energix/assets/placeholder.svg';

        // Újra próbálkozás placeholder-rel
        setTimeout(() => newImg.onload(), 100);
    };

    // Trigger kép betöltés
    newImg.src = imageData.src;

    // Thumbnail active state frissítése
    updateActiveThumbnail(imageData.thumbnailElement);
}

// Kép → Kép átmenet optimalizált kezelése  
function handleImageToImageTransition(imgElement, imageData) {
    console.log('📷 IMG frissítés (optimalizált)');

    // Preload új kép
    const preloadImg = new Image();

    preloadImg.onload = function() {
        console.log('✅ Új kép előtöltve, váltás indítása');

        // Smooth fade out
        imgElement.style.transition = 'opacity 0.15s ease';
        ...
        setTimeout(() => {
            ...
            console.log('✅ IMG váltás befejezve');
...
function updateActiveThumbnail(activeThumbnail) {
    ...
    console.log('✅ Active thumbnail frissítve');
}
