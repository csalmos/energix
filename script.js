...
function initProductPage(familyOrProductId, productId = null) {
    ...
    const mediaContainer = document.querySelector('.product-main-image');
    ...
    if (displayData.video) {
        ...
        mediaContainer.innerHTML = `
            <video 
                class="product-video"
                poster="${displayData.video.poster}"
                controls
                autoplay
                muted
                loop
                playsinline
            >
                <source src="${displayData.video.src}" type="${displayData.video.type}">
                <img src="${displayData.video.poster}" alt="${displayData.name}" />
            </video>
        `;
    } else if (displayData.images && displayData.images.length > 0) {
        // Kép renderelés fallback
        mediaContainer.innerHTML = `
            <img 
                src="${displayData.images[0].full}" 
