...
function initProductPage(familyOrProductId, productId = null) {
    ...
    if (displayData.video) {
        ...
        if (productImage.tagName === 'VIDEO') {
            console.log('🎬 Videó elem már létezik - nincs új videó létrehozás');
            return;
        }
        ...
        if (videoElement) {
            ...
            productImage.replaceWith(videoElement);
            
            // Hover autoplay csak sikeres betöltés után
            videoElement.addEventListener('canplay', () => {
                console.log('✅ Videó lejátszásra kész - hover autoplay aktiválás');
                setupHoverAutoplay(videoElement);
            });
            
        } else {
            console.error('❌ Videó létrehozás sikertelen - fallback kép marad');
        }
        ...
        videoElement.addEventListener('loadeddata', () => {
            console.log('✅ Videó betöltődött - hover autoplay aktív');
        });
        ...
        if (displayData.video) {
            ...
            videoThumbImg.addEventListener('click', function() {
                ...
                if (mainImage && mainImage.tagName === 'IMG') {
                    ...
                    setupHoverAutoplay(videoElement);
                    console.log('✅ Videó thumbnail váltás - hover autoplay aktív');
                }
...
// TELJES ADATBETÖLTÉS DIAGNOSZTIKA
function diagnoseProductData() {
    console.log('🔍 === PRODUCT DATA DIAGNOSZTIKA ===');
    
    // 1. window.products ellenőrzés
    console.log('🏪 window.products létezik:', typeof window.products);
    console.log('🏪 window.products:', window.products);
    
    // 2. currentDisplayData ellenőrzés  
    console.log('📊 window.currentDisplayData létezik:', typeof window.currentDisplayData);
    console.log('📊 currentDisplayData:', window.currentDisplayData);
    
    // 3. Aktuális termék adatai
    const currentUrl = window.location.pathname;
    console.log('🌐 Jelenlegi URL:', currentUrl);
    
    // 4. initProductPage paraméterek
    console.log('⚙️ Próbáljuk kinyerni a termék ID-t az URL-ből');
    
    // 5. Videó specifikus ellenőrzés
    if (window.currentDisplayData?.video) {
        console.log('🎬 Videó adat található:', window.currentDisplayData.video);
    } else {
        console.log('❌ NINCS VIDEÓ ADAT a currentDisplayData-ban');
        
        // Alternatív keresés
        Object.keys(window.products || {}).forEach(key => {
            const product = window.products[key];
            if (product.video) {
                console.log(`🎬 Videó találva termékben "${key}":`, product.video);
            }
        });
    }
    
    // 6. Script betöltés ellenőrzés
    const scripts = Array.from(document.querySelectorAll('script[src]'));
    const productDataScript = scripts.find(s => s.src.includes('product-data.js'));
    console.log('📜 product-data.js script:', productDataScript);
    console.log('📜 Script betöltve:', productDataScript ? 'IGEN' : 'NINCS');
}
