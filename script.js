...
window.selectRelatedProduct = function(button) {
    ...
    // Sikerüzenet
    showProductChangedNotification(productName);
    
    console.log('Termék váltás:', productName);
};

// Fő termék gombok frissítése
function updateMainProductButtons(productId, productName, productPrice) {
    const addToCartBtn = document.getElementById('addToCart');
    const buyNowBtn = document.getElementById('buyNow');
    
    // Snipcart attribútumok frissítése
    ...
    // Oldal címének frissítése (opcionális)
    const productTitle = document.querySelector('.product-name, .product-title');
    if (productTitle) {
    ...
    if (priceDisplay) {
        priceDisplay.textContent = formatPrice(productPrice);
    }
}

// Kis értesítés a termékváltásról
function showProductChangedNotification(productName) {
    ...
    `;
    notification.textContent = `✓ ${productName} kiválasztva`;
