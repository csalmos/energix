...
function initProductPage(productId) {
    ...
    // If not found, search within product families
    if (!product) {
        for (const family in window.products) {
            if (window.products[family].products && 
                window.products[family].products[productId]) {
                product = window.products[family].products[productId];
                console.log('Product found in family:', family);
