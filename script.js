...
function initProductPage(familyOrProductId, productId = null) {
    ...
    if (displayData.video) {
        // DUPLICATE CHECK - ha már videó elem van, ne csináljunk újat
        if (productImage.tagName === 'VIDEO') {
            console.log('⚠️ Videó elem már létezik, kihagyás');
            return;
        }
        
        // Új video elem létrehozása
        ...
        sourceElement.type = displayData.video.type;
        videoElement.appendChild(sourceElement);
        
        // IMG lecserélése VIDEO-ra
        productImage.parentNode.replaceChild(videoElement, productImage);
        
        console.log('✅ IMG tag lecserélve VIDEO tag-re');
        ...
        productImage.src = displayData.images[0].full;
        productImage.alt = displayData.name;

        // Error handling csak IMG elemre
    ...
    if (thumbnails && displayData.images && displayData.images.length > 0) {
        ...
        displayData.images.forEach((image, index) => {
            ...
            img.addEventListener('click', function() {
                console.log('🔥 CLICK HANDLER DEBUG START');

                // KRITIKUS: Global mutex check
                if (window.imageTransitionLock) {
                    console.log('🔒 Transition már folyamatban - ABORT');
                    return;
                }
                ...
                console.log('📷 Thumbnail kattintás:', this.alt);
                console.log('Main elem típusa:', mainElement.tagName);
                console.log('Parent node:', mainElement.parentNode.tagName);
    ...
    setTimeout(() => {
        ...
        console.log('✅ Nyíl navigáció beállítva');
    ...
    console.log('Actual product ID for assets:', actualProductId);
    ...
    setTimeout(() => {
        ...
        console.log('✅ Defenzív navigáció inicializálva');
...
function handleVideoToImageDefensive(videoElement, imageData, timeoutId) {
    console.log('🔄 VIDEO→IMG (defensive mode)');
    ...
    newImg.onload = function() {
        console.log('✅ Kép betöltődött - defensive csere');
        ...
        if (currentVideoElement && 
            ...
            try {
                console.log('🔄 DOM csere végrehajtása...');
                ...
                console.log('✅ VIDEO→IMG csere sikeres (defensive)');
                
            } catch (domError) {
                console.error('DOM csere hiba:', domError);
                
                // EMERGENCY: Container alapú fallback
        ...
        setTimeout(() => {
            window.imageTransitionLock = false;
            console.log('🔓 Transition unlock (VIDEO→IMG)');
        }, 300);
    };
...
// JAVÍTOTT onerror setup - garantált működés
function setupBulletproofErrorHandler(imgElement, imageData) {
    console.log('🛡️ Bulletproof error handler setup');
    
    // KRITIKUS: Error handler ELŐRE beállítása
    imgElement.onerror = function(errorEvent) {
        console.log('🚨 IMAGE ERROR DETECTED:', this.src);
        
        const failedUrl = this.src;
        
        // AZONNALI fallback chain
        if (failedUrl.includes('placeholder.svg')) {
            // Placeholder is failed - EMERGENCY!
            console.log('💀 PLACEHOLDER FAILED - EMERGENCY AKTIVÁLÁS');
            
            // Null check
            if (typeof handleEmergencyImagePlacement === 'function') {
                handleEmergencyImagePlacement(this, imageData);
            } else {
                console.error('🚨 handleEmergencyImagePlacement NOT FOUND - CREATING INLINE');
                createInlineEmergencyFallback(this, imageData);
            }
            
        } else {
            // First failure - try placeholder
            console.log('🔄 First failure - placeholder próbálkozás');
            this.src = '/energix/assets/placeholder.svg';
            
            // BIZTOSÍTÉK: Ha placeholder sem töltődik 3 sec alatt
            setTimeout(() => {
                if (!this.complete || this.naturalWidth === 0) {
                    console.log('⏰ PLACEHOLDER TIMEOUT - Force emergency');
    ...
    // IMMEDIATE error check - ha már hibás
    if (imgElement.complete && imgElement.naturalWidth === 0) {
        console.log('🚨 Image már hibás állapotban - azonnali error trigger');
        imgElement.onerror();
    }
    
    console.log('✅ Error handler beállítva');
}


// MARKETING-OPTIMALIZÁLT Emergency Handler
function handleEmergencyImagePlacement(newImg, imageData = null, timeoutId = null) {
    console.log('🆘 Emergency recovery aktiválva');
    
    // Analytics tracking - marketing adatok gyűjtése
    if (typeof gtag !== 'undefined') {
        gtag('event', 'emergency_image_fallback', {
            'event_category': 'UX_reliability',
            'value': 1,
            'custom_parameter': 'fallback_triggered'
        });
    }
    
    // Egyszerűsített, gyors recovery
    ...
    if (container) {
        // Brutális cleanup
        const oldElement = document.getElementById('productImage');
        if (oldElement) oldElement.remove();
        
        // Új elem instant behelyezés
        newImg.id = 'productImage';
        newImg.className = 'product-main-image';
        newImg.style.cssText = 'width: 100%; height: auto; opacity: 1;';
        
        container.appendChild(newImg);
        
        // Success notification
        console.log('✅ Emergency recovery sikeres');
        
        // User feedback (opcionális)
        showBriefNotification('Kép betöltve ✓');
    }
...
// Mini notification marketing UX-hez
function showBriefNotification(message) {
    const notification = document.createElement('div');
    notification.style.cssText = `
        position: fixed; top: 20px; right: 20px; z-index: 1000;
        background: #7ed957; color: white; padding: 8px 12px;
        border-radius: 4px; font-size: 12px; opacity: 0.9;
    `;
...
// HIÁNYZÓ FÜGGVÉNY - Add hozzá a kódodhoz
function handleImageToImageDefensive(imgElement, imageData, timeoutId) {
    console.log('📷 IMG→IMG (defensive mode)');
    ...
    preloadImg.onload = function() {
        console.log('✅ Új kép előtöltve (defensive)');
        ...
        if (currentImgElement && 
            ...
            try {
                console.log('🔄 IMG src frissítése...');
                ...
                console.log('✅ IMG→IMG váltás sikeres (defensive)');
            ...
            console.log('⚠️ IMG element már nem valid - emergency mode');
        ...
        setTimeout(() => {
            window.imageTransitionLock = false;
            console.log('🔓 Transition unlock (IMG→IMG)');
        }, 300);
    };
    
    preloadImg.onerror = function() {
        console.error('❌ Kép előtöltési hiba:', imageData.src);
...
function updateThumbnailStateDefensive(activeThumbnailElement) {
    try {
        ...
        if (activeThumbnailElement && activeThumbnailElement.classList) {
            ...
            console.log('✅ Active thumbnail frissítve (defensive)');
        } else {
...
function reinitializeNavigationCompletely() {
    console.log('🔄 Teljes navigáció újrainicializálás...');
    
    // 1. CLEANUP minden régi elem
    cleanupOldNavigation();
    
    // 2. Global lock reset
    window.imageTransitionLock = false;
    
    // 3. Thumbnail event handler-ek clean újragenerálása
    ...
    // 4. Késleltetett újrainicializálás
    setTimeout(() => {
        // Újra setupArrowNavigation() és thumbnail handler-ek
        setupArrowNavigation();
        console.log('✅ Navigáció teljesen újrainicializálva');
    }, 500);
}

// TELJES navigációs cleanup - régi elemek brutális törlése
function cleanupOldNavigation() {
    console.log('🧹 Régi navigációs elemek törlése...');
    
    // Minden régi nav-area eltávolítása
    document.querySelectorAll('.nav-area').forEach((element, index) => {
        console.log(`Régi nav-area ${index} eltávolítva`);
        element.remove();
    });
    
    // Inline style-os navigációs elemek keresése és törlése
    document.querySelectorAll('[style*="cursor: url"]').forEach((element, index) => {
        if (element.style.cursor.includes('data:image/svg+xml')) {
            console.log(`Inline nav elem ${index} eltávolítva`);
            element.remove();
        }
    });
    
    // Event listener-ekkel rendelkező elemek cleanup
    const potentialNavElements = document.querySelectorAll('[class*="nav"], [id*="nav"]');
    ...
    console.log('✅ Régi navigáció cleanup befejezve');
}


// HIÁNYZÓ FUNKCIÓ: Nyíl navigáció beállítása
function setupArrowNavigation() {
    console.log('🎯 Nyíl navigáció inicializálása...');
    ...
    leftNavArea.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('👈 Bal nyíl kattintás');
    ...
    rightNavArea.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('👉 Jobb nyíl kattintás');
    ...
    console.log('✅ Nyíl navigációs területek hozzáadva');
}

// KRITIKUS: Navigációs logika
function navigateToImage(direction) {
    console.log(`🔄 Navigáció indítása: ${direction}`);
    ...
    console.log(`📍 Navigáció: ${currentIndex} → ${nextIndex}`);
    
    // KULCS: Thumbnail click szimulálása
    const thumbnailImg = nextThumbnail.querySelector('img');
    if (thumbnailImg) {
        console.log('🎯 Thumbnail click szimulálása...');
        ...
        console.log('✅ Navigáció befejezve');
