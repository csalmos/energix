...
function initProductPage(familyOrProductId, productId = null) {
    ...
    if (displayData.video) {
        ...
        // ✅ PROGRESS BAR indítása az 5 másodperces várakozáshoz
        const container = document.getElementById('productImage').parentElement || document.getElementById('productImage');
        showHoverProgress(container);
        console.log('Indul');

        // 5 másodperc UTÁN - UGYANAZ a logika mint a thumbnail click-nél!
        window.videoAutoplayTimer = setTimeout(() => {
            const mainImage = document.getElementById('productImage');
        
            console.log('Leáll');
            if (mainImage && mainImage.tagName === 'IMG') {
                window.videoAutoplayTimer = null;
                hideHoverProgress(container);
    ...
    if (thumbnails && displayData.images && displayData.images.length > 0) {
        ...
        displayData.images.forEach((image, index) => {
            ...
            img.addEventListener('click', function() {
                ...
                // ✅ KRITIKUS: Timer és progress bar törlése
                if (window.videoAutoplayTimer) {
                    console.log('⏹️ Autoplay timer törölve (user kattintás)');
                    clearTimeout(window.videoAutoplayTimer);
                    window.videoAutoplayTimer = null;
        
                    // Progress bar eltávolítása
                    const container = document.getElementById('productImage')?.parentElement;
                    if (container) {
                        hideHoverProgress(container);
                    }
                }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleMaterialTabClick(e) {
        // Ellenőrizzük, hogy ténylegesen anyag opcióra kattintottak-e
        if (!e.target.closest('.material-option')) {
            return; // Ha nem anyag opcióra kattintottak, kilépünk
        }
...
document.addEventListener('snipcart.ready', function() {
    Snipcart.events.on('shipping.selected', function(shippingMethod) {
        console.log('Shipping method object:', shippingMethod);
        // Ellenőrizd a pontos mező neveket
        if (shippingMethod && shippingMethod.method) {
            console.log('Method:', shippingMethod.method);
        }

        if (shippingMethod.method.includes('GLS Csomagautomata')) {
            console.log('GLS Csomagautomata kiválasztva - popup megnyitása');
            // GLS térkép megnyitása
            const glsMap = document.getElementById('gls-map');
            if (glsMap) {
                glsMap.showModal();
            }
        }
...
if (glsMap) {
    glsMap.addEventListener('change', (e) => {
        ...
        console.log('Kiválasztott automata:', selectedLocker.name);
        console.log('Cím:', selectedLocker.contact.address);
        localStorage.setItem('selectedGLSLocker', JSON.stringify(selectedLocker));
    });
} else {
    console.error('GLS map element not found!');
}

// GLS választás után kapott adatok
const locker = {
    name:          selectedLocker.name,          // pl. "GLS Automat 123"
    street:        selectedLocker.contact.address.street,   // "Fő utca 1."
    city:          selectedLocker.contact.address.city,     // "Budapest"
    zip:           selectedLocker.contact.address.zip,      // "1054"
    country:       'HU'
  };
  
  // A cím frissítése a Snipcart checkoutban
  Snipcart.store.dispatch('shippingAddress.update', {
    name:        locker.name,          // cím 1. sora – GLS elnevezés
    address1:    locker.street,        // utca-házszám
    city:        locker.city,
    postalCode:  locker.zip,
    country:     locker.country
  });
  
  // Ha a vásárló NEM pipálta be, hogy külön szállítási cím kell,
  // a Snipcart automatikusan a shippingAddress-t másolja a billingAddress-be.
  // Így az új automata-cím a számlán is megjelenik.
