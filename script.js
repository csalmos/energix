...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Globális függvény az onclick számára
    window.selectRelatedProduct = function(button) {
        try {
            const suggestionItem = button.closest('.suggestion-item');
            if (!suggestionItem) {
                console.error('Nem található suggestion-item elem');
                return;
            }
            
            const productId = suggestionItem.getAttribute('data-product-id');
            const productName = suggestionItem.getAttribute('data-product-name');
            const productPrice = suggestionItem.getAttribute('data-product-price');
            
            console.log('Termékváltás:', productName, productPrice + ' Ft');
            
            // Fő gombok frissítése
            updateMainProductButtons(productId, productName, productPrice);
            
            // Vizuális feedback
            updateProductSelection(suggestionItem);
            
        } catch (error) {
            console.error('Hiba a termékválasztásban:', error);
        }
    };
    
    // Fő gombok frissítése
    function updateMainProductButtons(productId, productName, productPrice) {
        const addToCartBtn = document.getElementById('addToCart');
        const buyNowBtn = document.getElementById('buyNow');
        ...
        [addToCartBtn, buyNowBtn].forEach(btn => {
            if (btn) {
                ...
                btn.setAttribute('data-item-price', productPrice);
                
                // URL és kép frissítése is szükséges lehet
                const productUrls = {
                    '102': '/argentum',
                    '103': '/zafir'
                };
                ...
                const productImages = {
                    '102': 'https://energixwear.hu/assets/EnergiX/Argentum/Argentum-main.webp',
                    '103': 'https://energixwear.hu/assets/EnergiX/Zafir/Zafir-main.webp'
                };
                
                if (productUrls[productId]) {
                    btn.setAttribute('data-item-url', productUrls[productId]);
                }
                
                if (productImages[productId]) {
                    btn.setAttribute('data-item-image', productImages[productId]);
                }
            }
        });
        
        // Oldal adatok frissítése
        updatePageDisplay(productName, productPrice);
    }
    
    // Vizuális kiválasztás
    function updateProductSelection(selectedItem) {
        ...
        // Success üzenet
        showNotification('✓ ' + selectedItem.getAttribute('data-product-name') + ' kiválasztva');
    }
    
    // Oldal megjelenítés frissítése
    function updatePageDisplay(productName, productPrice) {
        const productTitle = document.querySelector('.product-name, .product-title, h1');
        if (productTitle) {
