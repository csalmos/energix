...
if (typeof CookieConsent === 'undefined') {
    const CookieConsent = {
        ...
        showBanner: function() {
            ...
            if (banner) {
                ...
                setTimeout(() => {
                    ...
                    document.body.style.marginBottom = banner.offsetHeight + 'px';
                }, 1000);
            }
        },
        ...
        // Toggle cookie settings details
        toggleDetails: function() {
            const details = document.getElementById('cookie-consent-details');
            const toggleBtn = document.querySelector('.cookie-consent-toggle');
        
            if (details && toggleBtn) {
                details.classList.toggle('show');
                const isExpanded = details.classList.contains('show');
                toggleBtn.textContent = isExpanded ? 'Kevesebb megjelenítése' : 'További információ a sütikről';
        ...
        saveConsent: function(consent) {
            ...
            const d = new Date();
            d.setTime(d.getTime() + (this.COOKIE_EXPIRY_DAYS * 24 * 60 * 60 * 1000));
            const expires = 'expires=' + d.toUTCString();
            document.cookie = `${this.CONSENT_COOKIE}=${JSON.stringify(consent)};${expires};path=/;SameSite=Lax`;
        
            // If user accepts, load additional scripts
            if (consent.necessary && consent.preferences && consent.statistics && consent.marketing) {
        ...
        getConsent: function() {
            const name = this.CONSENT_COOKIE + '=';
        ...
        // Load additional scripts that require consent
        loadAdditionalScripts: function() {
            // Add any additional scripts that require consent here
            // For example:
            // this.loadScript('https://example.com/analytics.js', 'analytics');
        },
        ...
        setupEventListeners: function() {
            ...
            const saveSettingsBtn = document.getElementById('cookie-consent-save');
            if (saveSettingsBtn) {
                saveSettingsBtn.addEventListener('click', () => {
                    const necessary = document.getElementById('cookie-necessary').checked;
                    ...
                    this.saveConsent({
                        necessary: true, // Always true as it's required for the site to function
