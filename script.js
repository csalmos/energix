...
function initProductPage(familyOrProductId, productId = null) {
    ...
    if (thumbnails && displayData.images && displayData.images.length > 0) {
        ...
        displayData.images.forEach((image, index) => {
            ...
            // JAVÍTOTT CLICK HANDLER - Video/Image kompatibilis
            img.addEventListener('click', function() {
                ...
                if (mainElement.tagName === 'VIDEO') {
                    console.log('🔄 VIDEO → IMG váltás');
            
                    // VIDEO lecserélése IMG-re
                    const newImg = document.createElement('img');
                    newImg.id = 'productImage';
                    newImg.className = 'product-main-image';
                    newImg.src = this.dataset.full;
                    newImg.alt = this.alt;
                    newImg.style.opacity = '0';
            
                    // Csere
                    mainElement.parentNode.replaceChild(newImg, mainElement);
                    ...
                    console.log('📷 IMG frissítés');
            
                    // Hagyományos IMG frissítés
                    mainElement.style.opacity = '0';
                    setTimeout(() => {
                        mainElement.src = this.dataset.full;
                        mainElement.alt = this.alt;
