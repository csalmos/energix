...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (path.includes('product.html') || 
        path.includes('/eter') || 
        path.includes('/zafir') || 
        path.includes('/argentum')) {
        
        // Handle different URL formats
        if (path.includes('/energix/')) {
            // Format: /energix/eter
        ...
        if (productId && window.products && window.products[productId]) {
            ...
            console.error('Product not found or invalid product ID:', productId);
...
function initProductPage(productId) {
    ...
    // Update the description element part
    ...
    if (descriptionElement) {
        ...
        console.warn('Description element not found. Tried selectors:',
            '.product-description p, .description p, .product-description');
    }

    const fullDescriptionElement = document.getElementById('productFullDescription') || 
        document.querySelector('.product-full-description');

    if (fullDescriptionElement) {
        fullDescriptionElement.innerHTML = product.fullDescription;
    } else {
        console.warn('Full description element not found. Tried selectors:',
...
function updateSnipcartQuantity(quantity) {
    ...
    if (buyNowBtn) {
        buyNowBtn.setAttribute('data-item-quantity', quantity);
    }
    
    console.log('Mennyiség frissítve:', quantity);
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Fő kosárba rakás gomb referencia
    const mainAddToCartBtn = document.getElementById('addToCart');
    const mainBuyNowBtn = document.getElementById('buyNow');
    
    // Alapértelmezett termék adatok
    const originalProductData = {
        id: mainAddToCartBtn.getAttribute('data-item-id'),
        name: mainAddToCartBtn.getAttribute('data-item-name'),
        price: mainAddToCartBtn.getAttribute('data-item-price'),
        url: mainAddToCartBtn.getAttribute('data-item-url'),
        image: mainAddToCartBtn.getAttribute('data-item-image')
    };
    
    // Cross-sell termék kiválasztása
    window.selectCrossSellProduct = function(button) {
        const productCard = button.closest('.cross-sell-product');
        const productId = productCard.getAttribute('data-product-id');
        const productName = productCard.getAttribute('data-product-name');
        const productPrice = productCard.getAttribute('data-product-price');
        
        // További adatok a termékhez (pl. URL, kép)
        const productData = getProductDataById(productId);
        
        // Fő gombok frissítése
        updateMainButtons({
            id: productId,
            name: productName,
            price: productPrice,
        ...
        // Vizuális feedback
        updateProductSelection(productCard);
        
        // Oldal tetejére scroll (termék információkhoz)
        document.querySelector('.product-hero').scrollIntoView({ 
            behavior: 'smooth',
            block: 'start'
        });
    };
    
    // Fő gombok adatainak frissítése
    function updateMainButtons(productData) {
        // Add to Cart gomb frissítése
        mainAddToCartBtn.setAttribute('data-item-id', productData.id);
        mainAddToCartBtn.setAttribute('data-item-name', productData.name);
        mainAddToCartBtn.setAttribute('data-item-price', productData.price);
        mainAddToCartBtn.setAttribute('data-item-url', productData.url);
        mainAddToCartBtn.setAttribute('data-item-image', productData.image);
        
        // Buy Now gomb frissítése
        mainBuyNowBtn.setAttribute('data-item-id', productData.id);
        mainBuyNowBtn.setAttribute('data-item-name', productData.name);
        mainBuyNowBtn.setAttribute('data-item-price', productData.price);
        mainBuyNowBtn.setAttribute('data-item-url', productData.url);
        mainBuyNowBtn.setAttribute('data-item-image', productData.image);
        
        // Termék név és ár frissítése a oldalon
        updateProductDisplay(productData);
        
        console.log('Termék váltás:', productData.name);
    }
    
    // Termék adatok lekérése ID alapján
    function getProductDataById(productId) {
        const productDatabase = {
            '102': {
                url: '/argentum',
                image: 'https://energixwear.hu/assets/EnergiX/Argentum/Argentum-main.webp'
            },
            '103': {
                url: '/zafir',
                image: 'https://energixwear.hu/assets/EnergiX/Zafir/Zafir-main.webp'
            }
            // További termékek...
        };
        
        return productDatabase[productId] || originalProductData;
    }
    
    // Termék megjelenítés frissítése
    function updateProductDisplay(productData) {
        // Termék név frissítése
        const productTitle = document.querySelector('.product-title, .product-name');
        if (productTitle) {
    ...
    // Vizuális kiválasztás frissítése
    function updateProductSelection(selectedCard) {
        // Összes termék deaktiválása
        document.querySelectorAll('.cross-sell-product').forEach(card => {
    ...
    // Carousel funkcionalitás
    window.slideCarousel = function(direction) {
        const carousel = document.getElementById('crossSellCarousel');
        const cardWidth = 330; // 300px kártya + 30px gap
    ...
    function updateCarouselIndicators() {
        // Implementáció az indikátorok státuszának frissítésére
    }
    
    // Reset funkció (ha szükséges)
    window.resetToOriginalProduct = function() {
        updateMainButtons(originalProductData);
        updateProductSelection(document.querySelector('.cross-sell-product[data-product-id="' + originalProductData.id + '"]'));
    };
});
