...
// Mobile/touch eszközökön egyszerűsített verzió
function isTouchDevice() {
    return 'ontouchstart' in window || navigator.maxTouchPoints > 0;
}

// ÚJ FUNKCIÓ: Hover-alapú autoplay 5 másodperc késleltetéssel
function setupHoverAutoplay(videoElement) {
    if (isTouchDevice()) {
        console.log('📱 Touch eszköz észlelve - hover autoplay kihagyva');
        
        // Mobile-on egyszerű tap-to-play
    ...
    // HOVER START - 5 másodperces timer indítása
    container.addEventListener('mouseenter', function() {
        if (userInteracted || isVideoPlaying) return;
        
        console.log('🖱️ Hover start - 5 sec timer indítása');
        
        // Töröljük a meglévő timer-t
        if (hoverTimer) {
        ...
        hoverTimer = setTimeout(() => {
            if (!userInteracted && !isVideoPlaying) {
                console.log('▶️ 5 másodperc eltelt - videó indítása');
                
                videoElement.play().then(() => {
                    isVideoPlaying = true;
                    console.log('✅ Hover autoplay sikeres');
                    
                    // Marketing analytics
                    if (typeof gtag !== 'undefined') {
                        gtag('event', 'hover_autoplay', {
                            'event_category': 'video_engagement',
                            'video_duration': videoElement.duration || 'unknown',
                            'trigger_method': 'hover_5sec'
                        });
                    }
                    
                }).catch(error => {
                    console.log('⚠️ Autoplay letiltva böngészőben:', error);
                    // Fallback: visual indicator hogy kattintásra indul
                    showVideoPlayPrompt(videoElement);
                });
            }
        }, 5000); // 5 másodperc = 5000ms
        
        // Visual feedback - subtle indicator
        showHoverProgress(container);
    });
    
    // HOVER END - timer törlése
    container.addEventListener('mouseleave', function() {
        if (hoverTimer) {
            clearTimeout(hoverTimer);
            hoverTimer = null;
            console.log('🖱️ Hover end - timer törölve');
        }
        
        // Progress indicator eltávolítása
        hideHoverProgress(container);
        
        // Ha nem volt user interakció, videó megállítása
        if (!userInteracted && isVideoPlaying) {
            videoElement.pause();
            videoElement.currentTime = 0; // Visszatekerés elejére
            isVideoPlaying = false;
            console.log('⏸️ Hover end - videó megállítva');
        }
    });
    
    // CLICK - user interakció jelzése
    container.addEventListener('click', function(e) {
        // Ha ez egy navigációs click, ne szakítsuk meg
        if (e.target.closest('.nav-area')) return;
        ...
        if (videoElement.paused) {
            videoElement.play();
            isVideoPlaying = true;
            console.log('👆 User click - videó indítása');
        } else {
            videoElement.pause();
            isVideoPlaying = false;
            console.log('👆 User click - videó szüneteltetése');
        }
        
        // Analytics
        if (typeof gtag !== 'undefined') {
            gtag('event', 'video_manual_control', {
                'action': videoElement.paused ? 'pause' : 'play'
            });
        }
    });
    
    console.log('✅ Hover autoplay konfiguráció kész');
}
