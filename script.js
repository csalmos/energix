...
document.addEventListener('DOMContentLoaded', function() {
    if (window.location.pathname.includes('product.html')) {
        initializePage();  // For product page
    } else if (window.location.pathname.endsWith('index.html') || 
               window.location.pathname.endsWith('/') || 
               window.location.pathname === '') {
        initializeHomePage();  // For homepage
    }
});
...
function setupFeatureDotPositions() {
    ...
    if (isHomePage) {
        // Default to 'argentum' for the homepage
        productType = 'eter';
    }
    ...
    featureDots.forEach(dot => {
        try {
            ...
            const positionsStr = dot.getAttribute('data-positions');
            
            if (!positionsStr) {
                console.warn('No data-positions attribute found on feature dot');
                allDotsInitialized = false;
                return;
            }
            
            let positions;
            try {
                const rawPositions = dot.getAttribute('data-positions');
                if (rawPositions === 'right' || rawPositions === 'left' || 
                    rawPositions === 'top' || rawPositions === 'bottom') {
                    // Handle the case where only direction is provided
                    positions = {
                        [productType]: {
                            lineDir: rawPositions,
                            top: '50%',
                            left: '50%',
                            lineLength: 80
                        }
                    ...
                    // Handle the normal JSON case
                    const positionsStr = rawPositions.replace(/'/g, '"');
            ...
            if (position) {
                ...
                if (line) {
                    ...
                    if (position.lineLength) {
                        ...
                        line.style.removeProperty('--line-length');
                    }
                ...
                if (line) {
                    ...
                    const lineLength = position.lineLength || 120;
    ...
    // Only mark as initialized if all dots were processed successfully
    if (allDotsInitialized) {
        window.featureDotsInitialized = true;
    } else {
        console.warn('Some feature dots failed to initialize, will retry...');
        setTimeout(setupFeatureDotPositions, 100);
    }
}
