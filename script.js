...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pathSegments.length >= 1) {
        ...
        // Ellenőrizzük, hogy ez ismert termék/családnév-e
        if (window.products && window.products[firstSegment]) {
            if (pathSegments.length === 1) {
                // /irisz vagy /eter formátum
                initProductPage(firstSegment);
            } else if (pathSegments.length === 2) {
                // /irisz/aura formátum (csak termékcsaládnál)
                initProductPage(firstSegment, pathSegments[1]);
            }
        } else {
            console.error('Unknown product/family:', firstSegment);
        }
    }
    // Check for homepage
...
function initProductPage(familyOrProductId, productId = null) {
    console.log('initProductPage called with:', { familyId, productId });
    ...
    console.log('Product page initialized with data:', displayData);
    
    // Set main image
    const mainImage = document.getElementById('productImage');
    if (mainImage && displayData.images && displayData.images.length > 0) {
        // Set first image as main
        mainImage.src = displayData.images[0].full || 'assets/placeholder.svg';
