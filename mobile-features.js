document.addEventListener('DOMContentLoaded', function() {
    ...
    featureDots.forEach((dot, index) => {
        ...
        // Add touch start handler for immediate response
        dot.addEventListener('touchstart', handleTouchStart, { passive: true });
        dot.addEventListener('touchend', handleTouchEnd, { passive: true });
    });
    ...
    function handleTouchEnd(e) {
        ...
        if (currentActiveDot === this && !isActive) {
            closeBottomSheet();
        ...
        if (currentActiveDot && currentActiveDot !== this) {
            currentActiveDot.classList.remove('active');
        }
            
        // Set the current active dot
        currentActiveDot = isActive ? this : null;
            
        // Show or hide bottom sheet based on active state
        if (isActive) {
            showBottomSheet(this.dataset.label);
        } else {
            closeBottomSheet();
        }
    }
    ...
    bottomSheet.addEventListener('touchmove', function(e) {
        ...
        if (deltaY > 10) {
            isSwiping = true;
            e.preventDefault();
            bottomSheet.style.transform = `translateY(${Math.min(deltaY, 100)}px)`;
        }
    }, { passive: false });
